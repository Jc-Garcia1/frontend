...
import { useCreateDepartament, useDepartaments, useDeleteDepartament, useUpdateDepartament } from "../api/departaments.queries";
        ...
        <div className="bg-slate-50">
            <header className="border-b bg-black">
                <div className="mx-auto max-w-5xl px-4 py-4 flex items-center justify-between">
                    <h1 className="text-xl font-semibold text-white">DEPARTAMENTS</h1>
            ...
            <main className="mx-auto max-w-5xl px-4 py-6 space-y-4">
                <form onSubmit={onSubmit} className="rounded-xl border bg-white p-4 space-y-3">
                    <p className="text-sm text-slate-600">
                        <b>Mutation (POST)</b>: crea departament y luego invalida cache para refrescar listado.
                    </p>
                                    ...
                                    <tr key={c.id} className="border-t">
                                        ...
                                        <td className="p-3">
                                            <div className="flex justify-center gap-2">
                                                ...
                                                <button
                                                    className="rounded-md border border-black px-3 py-1 text-black hover:bg-gray-400" onClick={() => onEdit(c)}
                                                >
                                                    Editar
                                                </button>

                                                <button
                                                    className="rounded-md border border-black px-3 py-1 text-black hover:bg-gray-400" disabled={deleteMut.isPending}
                                                    onClick={() => {
                                                        if (!confirm("¿Seguro que deseas borrar este departament?")) return;
